...
import { loginAdmin, loginSales, requestForgotPasswordOtp, resetPasswordWithOtp, resendForgotOtp, selectRole } from '../api/auth'
...
export default function Login() {
  ...
  const [multiRoleRoles, setMultiRoleRoles] = useState([])
  const [multiRoleOpen, setMultiRoleOpen] = useState(false)
  const [multiRoleUser, setMultiRoleUser] = useState(null)
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      if (res.data?.multiRole) {
        setMultiRoleRoles(res.data.roles)
        setMultiRoleUser(user)
        setMultiRoleOpen(true)
        return
      }
      if (!user?.id || (user?.role !== 'admin' && user?.activeRole !== 'admin')) throw new Error('Invalid admin login response')
  ...
  const onSubmitSales = async (e) => {
    ...
    try {
      ...
      if (!user?.id || (user?.role !== 'sales' && user?.activeRole !== 'sales')) throw new Error('Invalid sales login response')
      localStorage.setItem('user', JSON.stringify(user))
      window.dispatchEvent(new Event('auth:changed'))
